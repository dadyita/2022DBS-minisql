...
TEST(DiskManagerTest, BitMapPageTest) {
  ...
  // LOG(INFO) << num_pages << std::endl;
  for (uint32_t i = 0; i < num_pages; i++) {
    ASSERT_TRUE(bitmap->IsPageFree(i));
    // LOG(INFO) << i+1 << ":" << bitmap->IsPageFree(i) << std::endl;
  }
  uint32_t ofs;
  std::unordered_set<uint32_t> page_set;
  for (uint32_t i = 0; i < num_pages; i++) {
    // bool flag = bitmap->AllocatePage(ofs);
    // ASSERT_TRUE(flag);
    // LOG(INFO) << i+1 << ":" << flag << std::endl;
  ...
  // bool flag = bitmap->AllocatePage(ofs);
  // ASSERT_FALSE(flag);
  // LOG(INFO) << "Outside the loop of insertion:" << flag << std::endl;
  ASSERT_FALSE(bitmap->AllocatePage(ofs));
  // flag = bitmap->DeAllocatePage(233);
  // LOG(INFO) << "DeAllocatePage(233):" << flag << std::endl;
