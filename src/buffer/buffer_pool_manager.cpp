...
BufferPoolManager::~BufferPoolManager() {
  for (auto page:page_table_) FlushPage(page.first);
...
bool BufferPoolManager::IsPageFree(page_id_t page_id) {
   ...
   *  if (page_id == 0)
   *    return true;
   *  else if (page_id % (DiskManager::BITMAP_SIZE + 1) == 1)
   *    return true;
   *  // 将page_id从物理页转换为逻辑页
   *  return disk_manager_->IsPageFree((page_id - 1) / (DiskManager::BITMAP_SIZE + 1) * DiskManager::BITMAP_SIZE +
   *                                   (page_id - 1) % (DiskManager::BITMAP_SIZE + 1) - 1);
   */
