...
ValueType B_PLUS_TREE_INTERNAL_PAGE_TYPE::Lookup(const KeyType &key, const KeyComparator &comparator) const {
  ...
  while (left <= right) {//说明[left,right]中间还有元素
    ...
    if (comparator(key,KeyAt(mid))<0) right = mid - 1; //[left,mid-1]处继续搜
    else left = mid + 1; //[mid+1,right]处继续搜
...
int B_PLUS_TREE_INTERNAL_PAGE_TYPE::InsertNodeAfter(const ValueType &old_value, const KeyType &new_key,
  ...
  insert_index++;
  for (int i = GetSize(); i > insert_index; i--) array_[i] = array_[i-1];
...
void B_PLUS_TREE_INTERNAL_PAGE_TYPE::Remove(int index) {
  int size = GetSize();
  for(int i=index; i<size-1; i++) array_[i] = array_[i+1];
